 ...
 * Если в буфере присутствует дублирующая терминальная последовательность - она автоматически удаляется.
 * Если хотя бы один из элементов некорректен, или буфер неполный, то процесс останавливается с неудачным исходом.
 * @return RespDecodingResult Если результат парсинга неудачный - в поле #remaining будет находиться входной
 * буфер без изменений, а #value гарантирует значение std::monostate. В случае успеха, остаток буфера будет
 * пустым, а поле значения будет содержать данные, представляющие один из допустимых типов RESP.
